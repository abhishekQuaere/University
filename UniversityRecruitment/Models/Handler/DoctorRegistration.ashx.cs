...
namespace AyushastraShopping.Models.Handlers
{
    ...
    /// </summary>
    public class DoctorRegistration : IHttpHandler
    {
            ...
            try
            {
                ...
                if (context.Request.Files.Count > 0)
                {
                    ...
                    for (int i = 0; i < files.Count; i++)
                    {
                        ...
                        fname = Path.Combine(context.Server.MapPath("/ReadWriteData/RegisterationDoc/"), fname);
                        file.SaveAs(fname);


                        context.Response.ContentType = "text/plain";
                        string path = Obj.getDoctorFilePath();
                        context.Response.Write(path + "RegisterationDoc/" + flname);
